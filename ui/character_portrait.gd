...
func _draw() -> void:
	...
	match form_name:
		"frog":
			_draw_icon(FROG_TEXTURE, center)
		"bird":
			_draw_icon(BIRD_TEXTURE, center)
		"worm":
			_draw_icon(WORM_TEXTURE, center)
		_:
			_draw_human(center)


func _draw_icon (texture : Texture2D, center : Vector2) -> void : 
	var icon_side: float = min(size.x, size.y) * 0.9
